...
export default function TodoItem({ todo, onUpdate, onRemove, onTagClick }: TodoItemProps) {
  ...
  return (
    <li className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 gap-3">
      {/* Top row: checkbox, title, and action buttons */}
      <div className="flex items-start sm:items-center flex-grow gap-3">
        <input
          type="checkbox"
          checked={todo.isCompleted}
          onChange={handleToggleComplete}
          className="form-checkbox h-5 w-5 mt-1 sm:mt-0 text-blue-600 dark:text-blue-500 rounded focus:ring-blue-500 dark:focus:ring-blue-400 border-gray-300 dark:border-gray-600 flex-shrink-0"
        />
        <div className="flex-grow min-w-0">
          {isEditing ? (
            <input
              ...
              className="w-full border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 text-gray-900 dark:text-white px-2 py-1 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              autoFocus
            />
          ) : (
            <span
              onDoubleClick={handleTitleDoubleClick}
              className={`block text-base sm:text-lg font-medium break-words ${
                todo.isCompleted
          ...
          {/* Tags and badges - shown below title on mobile */}
